...
[15장 코틀린 코루틴 테스트하기](#코틀린-코루틴-테스트하기)  
[16장 채널](#채널)  
[17장 셀렉트](#셀렉트)  
...
코틀린 코루틴 라이브러리의 모든 코루틴 빌더는 자신만의 Job을 생성한다.  
...
자식(launch 빌더)은 인자로 들어온 Job을 부모로 사용하기에 runBlocking과의 부모-자식 관계가 사라진다.   
...
runBlocking과의 부모-자식관계를 끊는다. 부모 코루틴이 없는 Job은 일반 Job과 동일하게 작동한다.
...
delay를 가상 시간 동안 실행하여 실제 시간이 흘러간 상황과 동일하게 작동하게 만든다.

코루틴에서 TestCoroutineScheduler를 사용하려면, 지원하는 디스패처를 사용해야 한다.   
일반적으로 StandardTestDispatcher를 사용한다. 테스트 디스패처로 시작된 코루틴은 가상 시간만큼 진행되기 전까지 실행되지 않는다.  
일반적인 방법으로 실제 시간처럼 작동하는 가상 시간을 흐르게 하여, 그 시간 동안 호출되었을 모든 작업을 실행하는 advanceUntilIdle을 사용하는 방식과 advanceTimeBy에 일정 밀리초 인자를 넣어
그동안 일어났을 모든 연산을 수행하게 하는 방식이 있다.    
기본적으로 StandardTestDispatcher는 TestCoroutineScheduler를 만들기에 명시적으로 만들지 않아도 된다.(scheduler 프로퍼티로 해당 스케줄러에 접근 가능하다.)

> 가상 시간은 어떻게 작동할까?   
> delay가 호출되면 디스패처가 Delay 인터페이스를 구현했는지 확인한다. 디스패처에서는 실제 시간만큼 기다리는 DefaultDelay 대신 디스패처가 가진 scheduleResumeAfterDelay 함수를
> 호출한다.

StandardTestDispatcher를 스코프로 래핑해서 사용하는 방식 대신 같은 역할을 수행하는 TestScope를 사용할 수 있다.  
스코프가 사용하는 스케줄러에 advanceUntilIdle, advanceTimeBy, currentTime 프로퍼티가 위임되기에 TestScope만으로 함수와 프로퍼티 모두를 사용할 수 있다.

### runTest

runTest는 kotlinx-coroutines-test 함수들 중 가장 흔하게 사용된다.  
TestScope에서 코루틴을 시작하고 즉시 유휴 상태가 될 때까지 시간을 흐르게 한다.  
코루틴에서는 스코프가 TestScope 타입이므로 아무때나 currentTime을 사용할 수 있다.

포함 관계(넓은 범위 -> 좁은 범위) : runTest > TestScope > StandardTestDispatcher > TestCoroutineScheduler

### 백그라운드 스코프

runTest 함수는 자식 코루틴이 끝날 때까지 기다린다. 절대 끝나지 않는 프로세스를 시작한다면 테스트도 종료되지 않는다.
이런 경우를 대비해, backgroundScope를 제공한다.
...
특정 함수가 구조화된 동시성을 지키고 있는지 테스트하려면, 중단 함수로부터 컨텍스트를 받은 뒤, 컨텍스트가 기대한 값을 가지고 있는지와 Job이 적절한 상태인지 확인하는 것이 가장 쉬운 방법이다.  
중단 함수에서 컨텍스트를 확인하려면, currentCoroutineContext 함수나 coroutineContext 프로퍼티를 사용하면 된다.  
코루틴 빌더의 람다식이나 스코프에서는 currentCoroutineContext 함수를 사용한다.   
이유는 CoroutineScope의 coroutineContext 프로퍼티가 현재 코루틴 컨텍스트를 제공하는 프로퍼티보다 우선하기 때문이다.

취소를 확인하는 가장 쉬운 방법은 내부 함수에서 job을 참조하고, 외부 코루틴에서 취소한 뒤, 참조된 Job이 취소된 것을 확인하는 것이다.
...
UnconfinedTestDispatcher는 코루틴을 시작했을 때 첫 번째 지연이 일어나기 전까지 모든 연산을 즉시 수행한다.

### 목(mock) 사용하기

Fake 객체에서 delay를 사용하기는 쉽지만, 명확하게 드러나지 않는다.  
mock을 사용해서 delay를 명확하게 드러낼 수 있다.
...
runBlocking으로 래핑된 테스트에서 호출하는 함수를 모킹하여, 사용한 스레드의 이름을 가지고 오는 방법으로 변경된 디스패처인지 테스트할 수 있다.  
디스패처를 바꾸는 함수에서 시간 의존성을 테스트해야 하는 경우도 있다.
이런 경우 StandardTestDispatcher를 새로운 디스패처가 대체하면 가상 시간에서의 작동을 멈추게 된다.
...
위 코드에서 시간테스트를 하려고 하면 모든 테스트가 실제 시간만큼 기다리고 currentTime은 0으로 유지된다.  
이 문제를 해결하는 가장 쉬운 방법은 생성자를 통해 디스패처를 주입 받는 것이다.
...
실행 중 프로그레스 바를 먼저 보여주고 나중에 숨기는 함수를 테스트 해보자.  
이런 경우, 함수를 새로운 코루틴에서 시작하고 바깥에서 가상 시간을 조절하는 방법이 도움이 된다.  
runTest는 코루틴의 디스패처로 StandardTestDispatcher를 지정하며 대기 상태가 될 때까지 시간이 흐르게 한다.  
자식 코루틴은 함수 본체의 실행이 끝났을 때 시간이 흐르게 된다.
...
단위 테스트에는 메인 함수가 없어 Dispatchers.Main를 사용할 수 없다.  
매번 메인 스레드를 주입하는 건 비용이 많이 들기에 `kotlinx-coroutines-test` 라이브러리는 Dispatchers에 setMain 확장 함수를 제공한다.  
테스트 시작 함수(@Before, @BeforeEach)에서 메인 함수를 설정하고 테스트 종료(@After, @AfterEach)시에 Dispatchers.resetMain()으로 메인 함수 상태를 초기화 시켜
테스트에서 메인 디스패처를 사용할 수 있다.

### 코루틴을 시작하는 안드로이드 함수 테스트하기

ViewModel을 테스트할 때, viewModelScope 또는 커스텀한 스코프를 사용해서 코드가 구현되어 있는 경우가 많다.  
스코프의 일부로 StandardTestDispatcher를 사용하면 되는데, 이 함수들 모두 Dispatcher 주입을 해서 테스트하기는 어렵다.  
조금 더 간단한 방법으로 안드로이드는 기본 디스패처로 Dispatchers.Main을 사용하고 있는데, Dispatchers.setMain 함수를 사용하면 디스패처를 StandardTestDispatcher로 교체할
수 있다.
...
@After
fun tearDown() {
    Dispathcers.resetMain()
}
```

scheduler를 통해 advanceTimeBy 함수와 advanceUntilIdle, runCurrent 함수들을 사용할 수 있게 된다.

### 룰이 있는 테스트 디스패처 설정하기

JUnit4는 Rule class 사용을 허용한다.
Rule은 테스트 클래스의 수명 동안 반드시 실행되어야 할 로직을 포함하는 클래스다.
...
class MainCoroutineRule : TestWatcher() {
    ...
    override fun finished(description: Description) {
        Disptachers.resetMain()
    }
}
```

JUnit5에서도 BeforeEachCallback, AfterEachCallback을 구현해서 Rule을 만들 수 있다.
...
안드로이드에서 Dispatchers.Main의 스레드가 블로킹되면 앱 실행이 멈추게 되고 Dispatchers.Default 스레드가 블로킹되면 프로세서를 효율적으로 사용하지 못하게 된다.  
...
CPU 집약적 연산이 필요하다면 Dispatchers.Default, 메인 뷰를 다루는 연산이 필요하다면 Dispatchers.Main.immediate 에서 실행해야 한다.
...
하지만 값의 계산 과정을 중단하려면 suspend getter 함수가 필요하며, suspend 프로퍼티도 필요하다.  
...
하지만 완료 중이거나 완료된 Job에서 새로운 코루틴을 시작할 수는 없다.  
